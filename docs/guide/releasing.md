...
`@layera-labs/react` declares `"@layera-labs/agentic": "workspace:*"` locally and
...
`@layera-labs/video/dist/karaoke.js` outright. That is what `beta.2` shipped and
...
`@layera-labs/video/browser` to prove its vendored mirrors still match the
