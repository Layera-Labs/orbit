...
- Add `@layera-labs/orbit-agentic` action types and adapter method for canvas-agent actions if the package owns backend contracts; otherwise keep the types internal to `@layera-labs/orbit-react`.
...
- Run `pnpm exec vitest run --passWithNoTests`, `pnpm test`, and `pnpm --filter @layera-labs/orbit-demo build`.
