...
| AI script/template agent           | **Library exists, mobile flow not connected** | `@layera-labs/orbit-video-ai` can create a limited template specification, but the current mobile AI path does not expose it through the render service. |
...
The render service is an Express application using the `@layera-labs/orbit-video` FFmpeg
renderer, `@layera-labs/orbit-video-gen` providers, `@layera-labs/orbit-auth`, and `@layera-labs/orbit-billing`.
...
| Check                                           | Result                                                                                                                      |
| ----------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| `pnpm --filter @layera-labs/orbit-video test`               | **83 tests passed** (was 81; +2 for local-FX alpha and NaN guards).                                                         |
| `pnpm --filter @layera-labs/orbit-video-gen test`           | **26 tests passed** across 4 files.                                                                                         |
| `pnpm --filter @layera-labs/orbit-render-service test`      | **15 tests passed** (was 10; +5 for `collectClientSrcs`); 3 Postgres tests skipped because `TEST_DATABASE_URL` was not set. |
| `pnpm --filter @layera-labs/orbit-render-service typecheck` | **Passed**.                                                                                                                 |
