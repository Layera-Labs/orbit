...
- **AI wedge** — [`@orbit/video-ai`](packages/video-ai): a pluggable **`Brain`** picks + fills a template → `buildProjectFromSpec` → render. Built-in **Gemini** brain (`@google/genai`, structured JSON output); the LLM is swappable. Unit-tested without a key (mock brain); live `describe → video` runs with `GEMINI_API_KEY`. **Media generation** (nano-banana / Seedance / Runway / gpt-image) plugs into `@orbit/video-gen`'s `MediaProvider` — deferred until keys are wired.
...
- **Render service** — [`@orbit/render-service`](services/render): `POST /v1/render { project }` → server-side ffmpeg → MP4. The spine the iOS app + web call. Verified by curl.
