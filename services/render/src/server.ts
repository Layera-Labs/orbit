 ...
 * else's account.
 *
 * Generation is credit-metered via `@layera-labs/orbit-billing`: each account is granted
...
import {
  ...
  type VideoProject,
  type WordTiming,
} from "@layera-labs/orbit-video/node";
import {
  ElevenLabsProvider,
  GenerationService,
  ProviderError,
  RunwayProvider,
  groupWords,
} from "@layera-labs/orbit-video-gen";
import {
...
import { authFromEnv, AuthError, type UserStore } from "@layera-labs/orbit-auth";
...
import { formatById } from "@layera-labs/orbit-formats";
...
export function createServer(): Express {
   ...
   * a promise through `@layera-labs/orbit-video`'s whole argument builder. This keeps the
   ...
   * ---- watching a render ----
   *
   * `ExportJobPoller` in `@layera-labs/orbit-core` has been written against an SSE endpoint
  ...
  app.post("/v1/transcribe", aiLimit, async (req: Request, res: Response) => {
    ...
    try {
      ...
      /*
       * Annotated, not inferred. `groupWords` lives in `@layera-labs/orbit-video-gen` and
       * `CaptionLine` in `@layera-labs/orbit-video`, two packages that do not depend on
