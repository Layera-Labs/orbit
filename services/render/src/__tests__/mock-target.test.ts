...
// about it. When `@orbit/video` was split and the service moved to
// `@orbit/video/node`, all seven suites here kept mocking the old name — so
...
/** Which `@orbit/*` specifiers `server.ts` really imports from. */
const imported = new Set(
  [...server.matchAll(/from\s*["'](@orbit\/[^"']+)["']/g)].map((m) => m[1]),
);

/** Every `vi.mock('@orbit/…')` across the suites in this directory. */
...
describe('the suites mock what the server imports', () => {
  ...
  it('mocks the render entry, not the browser one', () => {
    // The specific swap that happened. `@orbit/video` is browser-only now and
    ...
    expect(imported.has('@orbit/video/node')).toBe(true);
    expect([...imported]).not.toContain('@orbit/video');
  });
});
