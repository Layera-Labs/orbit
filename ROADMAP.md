...
| React Native | ❌ | ◐ (typed bridge core; WebView shell + native pending) |
...
- **B — React Native embed (the RN-first deliverable), on v2.** New `@orbit/react-native` loads the v2 web build in `react-native-webview`; a **typed JS bridge** exposes the public SDK API (`loadDocument`, `export`, `onChange`, `applyOps`); native chrome (image/video picker, camera, share sheet, file save) lives RN-side. Verified on a real mid-range Android, not just the simulator.
  - _Landed 2026-06-25:_ the typed **bridge core** — `@orbit/react-native` with the message protocol, host bridge (drives an `OrbitStore`: ops/export/undo/select + change/selection/history events), client API (`loadDocument`/`applyOps`/`export`/`onChange`…), and transports (in-memory for tests + RN/WebView factories). Verified end-to-end over an in-memory transport (7 tests). **Remaining:** the `<WebView>` shell component + the single-file web build it loads, native pickers/camera/share, and on-device Android verification (see `packages/react-native/README.md`).
