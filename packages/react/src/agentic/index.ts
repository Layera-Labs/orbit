/**
 * `@layera-labs/orbit-react/agentic` — the AI half, behind a door you have to open.
 *
 * `useOrbitAgentic` used to sit on the main barrel (`src/hooks/index.ts`, which
 * `src/index.ts` re-exports wholesale), so `import { … } from '@layera-labs/orbit-react'`
 ...
 * `@layera-labs/orbit-agentic` is an OPTIONAL peer (see this package's `package.json`) — a
 * host that declined it still installs, builds, bundles AND TYPECHECKS
 * `@layera-labs/orbit-react`, and simply never imports this module.
 *
 * This entry is the one place where naming `@layera-labs/orbit-agentic` would be
 * legitimate, because depending on it here is the point. As of the
 * `@layera-labs/orbit-shared` move nothing under it needs to: the canvas-agent types come
 * from `@layera-labs/orbit-shared` now, which is why the whole package — subpath included
 * — is currently free of the specifier.
 *
 * `src/__tests__/ai-optional.test.ts` walks the main entry's import graph and
 * fails if a RUNTIME import of `@layera-labs/orbit-agentic` reappears there, and separately
