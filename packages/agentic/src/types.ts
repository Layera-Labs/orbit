 ...
 * Agentic types
 *
 * The canvas-agent shapes moved to `@layera-labs/orbit-shared` and are re-exported from
 * here, so this package's public surface is unchanged for anyone already
 * writing `import type { CanvasAgentParams } from '@layera-labs/orbit-agentic'`.
 *
 * Why they had to move: `@layera-labs/orbit-react` NAMES them — `AiBackend.runCanvasAgent`
 ...
 * nothing. `@layera-labs/orbit-shared` is a real dependency of both packages, which is what
 * makes it the one place both can point at.
 *
 * What deliberately did NOT move is below: `AgenticConfig` and this package's
 * `ModelProvider`. They configure the AI CLIENT, `@layera-labs/orbit-react` never names
 * either, and moving `ModelProvider` would in fact break things — `@layera-labs/orbit-shared`
