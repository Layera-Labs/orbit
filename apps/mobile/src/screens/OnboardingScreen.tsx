...
export function OnboardingScreen({ onContinue }: { onContinue: () => void }) {
  ...
  return (
    <LinearGradient colors={['#ffffff', '#f1efff']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.root}>
      <ScrollView contentContainerStyle={styles.body} showsVerticalScrollIndicator={false}>
        <View style={styles.brandBlock}>
          ...
          <Text style={styles.tagline}>Powerful video editing.{`\n`}Open for everyone.</Text>
        </View>

        <View style={styles.actions}>
          <PrimaryButton label='Get Started' onPress={onContinue} />
          <Pressable accessibilityRole='button' style={styles.guest} onPress={onContinue}>
            <VIcon name='navHome' size={18} color={vela.ink2} />
            <Text style={styles.guestText}>Continue as Guest</Text>
          </Pressable>
        </View>

        <Pressable style={styles.promise} onPress={() => setWhyOpen(true)} hitSlop={8}>
...
function WhyOrbitSheet({ onClose }: { onClose: () => void }) {
  ...
  return (
    <BottomSheet onClose={onClose} style={styles.whySheet} dim='#0008'>
      ...
      {rows.map((row) => (
        <View key={row.title} style={styles.whyRow}>
          <View style={[styles.whyIcon, { backgroundColor: `${row.color}26` }]}>
            <VIcon name={row.icon} size={22} color={row.color} strokeWidth={2.1} />
          </View>
          <View style={{ flex: 1 }}>
...
const styles = StyleSheet.create({
  ...
  whyTitle: { color: '#fff', fontFamily: font.extrabold, fontSize: 25 },
  ...
  whyRowTitle: { color: '#fff', fontFamily: font.bold, fontSize: 16 },
  whyRowDetail: {
    color: vela.muted,
    fontFamily: font.medium,
    fontSize: 13,
