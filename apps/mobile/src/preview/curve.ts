 ...
 * ffmpeg expression and runs on the server, which has the canonical copy — but
 * `curvePoints` IS here, because a stored envelope has to become the same point
 * list on this screen as it does in the file.
 */
import type {
  VolumeCurve,
  VolumeDuck,
...
export function clipGainAt(
  ...
  // The plateau has to travel with the curve: an envelope is drawn AGAINST the
  // clip"s level, so materializing it against 1 would play a ducked bed at
  // unity while the exported file plays it at the level the user set.
  const pts = curvePoints(clip.volumeCurve, clip.duration ?? 1, vol);
  return pts ? Math.max(0, sampleVolume(pts, p)) : vol;
}
