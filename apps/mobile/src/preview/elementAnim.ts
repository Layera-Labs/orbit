/**
 * Mirror of `packages/video/src/element-anim.ts` — entrance and exit animation
 * for any element.
 *
 * A copy because `apps/mobile` installs outside the pnpm workspace and cannot
 * import `@orbit/video`; `__tests__/elementAnim.test.ts` imports the canonical
 * module by relative path and compares OUTPUT, which is the arrangement every
 * other shared function in this directory lives under.
 *
...
import type { AnimKind, ElementAnim, SlideEdge } from '../model/types';

export type { AnimKind, ElementAnim, SlideEdge } from '../model/types';
