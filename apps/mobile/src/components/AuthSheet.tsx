...
function friendly(e: unknown): string {
  if (e instanceof AuthError) {
    switch (e.kind) {
      ...
      case 'invalid-token': return 'That reset code is invalid or has expired. Request a new one.';
      case 'email-unconfigured': return 'This server can’t send email yet, so password reset is unavailable.';
...
export function AuthSheet({ onClose, onAuthed }: { onClose: () => void; onAuthed?: () => void }) {
  ...
  const heading = isLogin ? 'Sign in' : isRegister ? 'Create account' : isForgot ? 'Reset password' : 'Set a new password';
  const sub = isLogin || isRegister
    ? 'An account is needed to generate with AI and track credits.'
    : isForgot
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.fill}>
          <ScrollView contentContainerStyle={styles.body} keyboardShouldPersistTaps="handled">
            ...
            <Pressable
              ...
              style={[styles.primary, (isForgot ? !canForgot : isReset ? !canReset : !canAuth) && { opacity: 0.5 }]}
            >
              {busy ? (
                <ActivityIndicator color={vela.onAccent} />
              ) : (
                <Text style={styles.primaryText}>
                  {isLogin ? 'Sign in' : isRegister ? 'Create account' : isForgot ? 'Send reset code' : 'Reset password'}
                </Text>
              )}
            </Pressable>
...
const styles = StyleSheet.create({
  ...
  title: { flexShrink: 1, color: '#fff', fontFamily: font.extrabold, fontSize: 30, letterSpacing: -0.5 },
  close: { padding: 2 },
  subtitle: { color: vela.muted, fontFamily: font.medium, fontSize: 14.5, marginTop: -4, lineHeight: 20 },
  info: { color: vela.textLight2, fontFamily: font.medium, fontSize: 13.5, lineHeight: 19 },
  input: {
  ...
  code: { fontFamily: Platform.OS === 'ios' ? 'Menlo' : 'monospace', fontSize: 13, minHeight: 78, textAlignVertical: 'top' },
  forgotLink: { alignSelf: 'flex-end', paddingVertical: 2 },
  forgotText: { color: vela.textLight2, fontFamily: font.semibold, fontSize: 13.5 },
  ...
  socialText: { color: vela.textLight, fontFamily: font.semibold, fontSize: 15.5 },
  toggle: { alignSelf: 'center', paddingVertical: 10, paddingHorizontal: 12, marginTop: 2 },
