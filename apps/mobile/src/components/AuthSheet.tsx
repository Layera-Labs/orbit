...
function GoogleMark() {
  return (
    <Svg width={18} height={18} viewBox='0 0 24 24'>
      <Path fill='#4285F4' d='M23.06 12.25c0-.85-.08-1.67-.22-2.45H12v4.64h6.2c-.27 1.44-1.08 2.66-2.3 3.48v2.9h3.72c2.18-2 3.44-4.96 3.44-8.57z' />
      <Path fill='#34A853' d='M12 24c3.11 0 5.72-1.03 7.62-2.79l-3.72-2.9c-1.03.69-2.35 1.1-3.9 1.1-3 0-5.54-2.03-6.45-4.75H1.7v2.99C3.59 21.38 7.5 24 12 24z' />
      <Path fill='#FBBC05' d='M5.55 14.66c-.23-.69-.36-1.42-.36-2.16s.13-1.47.36-2.16V7.35H1.7C.95 8.85.5 10.55.5 12.5s.45 3.65 1.2 5.15l3.85-2.99z' />
      <Path fill='#EA4335' d='M12 4.75c1.69 0 3.21.58 4.4 1.72l3.3-3.3C17.71 1.28 15.1.25 12 .25 7.5.25 3.59 2.87 1.7 6.65l3.85 2.99C6.46 6.92 9 4.75 12 4.75z' />
    </Svg>
  );
}
...
export function AuthSheet({ onClose, onAuthed }: { onClose: () => void; onAuthed?: () => void }) {
  ...
  const submitForgot = async () => {
    ...
    try {
      ...
      setInfo(`If an account exists for ${email.trim()}, a reset code is on its way. Paste it below to set a new password.`);
      setPassword('');
      setView('reset');
    } catch (e) {
      setErr(friendly(e));
  ...
  const heading = isLogin ? 'Sign in to unlock AI features' : isRegister ? 'Create your AI account' : isForgot ? 'Reset password' : 'Set a new password';
  const sub = isLogin || isRegister ? 'Orbit stays open for editing and export. Sign in only when you want AI Studio.' : isForgot ? 'Enter your email and we’ll send a reset code.' : 'Paste the code from your email and choose a new password.';
      ...
      <View style={styles.root}>
        <KeyboardAvoidingView behavior={process.env.EXPO_OS === 'ios' ? 'padding' : undefined} style={styles.fill}>
          <ScrollView contentContainerStyle={styles.body} keyboardShouldPersistTaps='handled'>
            ...
            {isReset ? <TextInput style={[styles.input, styles.code]} value={token} onChangeText={setToken} placeholder='Reset code' placeholderTextColor={vela.muted3} autoCapitalize='none' autoCorrect={false} multiline /> : <TextInput style={styles.input} value={email} onChangeText={setEmail} placeholder='Email' placeholderTextColor={vela.muted3} keyboardType='email-address' autoCapitalize='none' autoCorrect={false} autoComplete='email' textContentType='emailAddress' returnKeyType={isForgot ? 'send' : 'next'} onSubmitEditing={isForgot ? submitForgot : undefined} />}

            {!isForgot ? <TextInput style={styles.input} value={password} onChangeText={setPassword} placeholder={isLogin ? 'Password' : 'Password (8+ characters)'} placeholderTextColor={vela.muted3} secureTextEntry autoCapitalize='none' autoComplete='password' textContentType={isLogin ? 'password' : 'newPassword'} returnKeyType='go' onSubmitEditing={isReset ? submitReset : submitAuth} /> : null}

            {isLogin ? (
              <Pressable onPress={() => go('forgot')} hitSlop={8} style={styles.forgotLink}>
            ...
            <Pressable
              onPress={isForgot ? submitForgot : isReset ? submitReset : submitAuth}
            ...
            >
              {busy ? <ActivityIndicator color={vela.onAccent} /> : <Text style={styles.primaryText}>{isLogin ? 'Sign in' : isRegister ? 'Create account' : isForgot ? 'Send reset code' : 'Reset password'}</Text>}
            </Pressable>
            ...
            {isLogin || isRegister ? (
              <>
                ...
                <Pressable style={styles.social} onPress={() => setSocial('Apple sign-in is coming soon.')}>
                  <AppleMark color={vela.textLight} />
                  <Text style={styles.socialText}>Continue with Apple</Text>
                </Pressable>
                <Pressable style={styles.social} onPress={() => setSocial('Google sign-in is coming soon.')}>
                  <GoogleMark />
                  <Text style={styles.socialText}>Continue with Google</Text>
                </Pressable>

                <Pressable onPress={() => go(isLogin ? 'register' : 'login')} hitSlop={8} style={styles.toggle}>
              ...
              <Pressable onPress={() => go('login')} hitSlop={8} style={styles.toggle}>
                <Text style={styles.toggleText}>
                  {isReset ? 'Didn’t get a code? ' : 'Remembered it? '}
                  <Text style={styles.toggleAccent}>{isReset ? 'Start over' : 'Back to sign in'}</Text>
                </Text>
              </Pressable>
            )}
          </ScrollView>
...
const styles = StyleSheet.create({
  root: { flex: 1, backgroundColor: vela.editorBg, paddingTop: 62 },
  fill: { flex: 1 },
  ...
  orLine: { flex: 1, height: 1, backgroundColor: vela.divider },
  orText: { color: vela.muted2, fontFamily: font.medium, fontSize: 13 },
  social: {
    height: 52,
    borderRadius: 14,
    backgroundColor: vela.card2,
