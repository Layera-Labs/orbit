 ...
 * cannot reach. The matrix itself comes from `gradeMatrix` in `@layera-labs/orbit-video`,
 ...
 */
import { gradeMatrix, type FilterParams } from '@layera-labs/orbit-video/browser';
...
export function filterFor(f: FilterParams): string | null {
  ...
  /*
   * The whole grade — eq AND colortemperature — comes from `@layera-labs/orbit-video`.
   *
